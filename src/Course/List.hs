...
headOrAgain ::
  ...
  -> List a
  -> a
headOrAgain d = foldRight const d
...
find ::
  ...
  -> Optional a
find f xs =
  headOr Empty (map (Full) (filter f xs))
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 xs = (length (myTake 5 xs)) == 5
    where myTake _ Nil = Nil
          myTake 0 _ = Nil
...
reverse'' ::
    ...
    -> List a
    -> List a
reverse'' acc xs = foldLeft (\x y -> y:.x) Nil xs
