...
failed ::
  ParseError
  -> Parser a
failed err = P (\_ -> ErrorResult err)
...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f (P pfa) = P (\i -> case (pfa i) of
                    ErrorResult c -> ErrorResult c
...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser apb (P pfa) = P (\i -> case (pfa i) of
                            ErrorResult err -> ErrorResult err
                            Result remaining a -> parse (apb a) remaining)
...
(>>>) ::
  ...
  -> Parser b
  -> Parser b
P pfa >>> pb = P (\i -> case (pfa i) of
                         ErrorResult err -> ErrorResult err
                         Result remaining _ -> parse pb remaining)
...
(|||) ::
  ...
  -> Parser a
P pfa ||| pb = P (\i -> case pfa i of
               ErrorResult _ -> parse pb i
               Result remaining a -> Result remaining a)
...
-- list pa@(P pfa) = P (\i -> case pfa i of
--                      ErrorResult _ -> Result i Nil
--                      Result remaining a -> parse (mapParser (a :.) (list pa)) remaining)
...
{- CONVERTING BINDS TO DO NOTATION
    1. insert the keyword do
    2. turn >>= into <-
    3. delete ->
    4. delete \ (and unnecessary brackets)
    5. swap each side of <-
-}
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser = foldRight (\pa pacc -> do
        a <- pa
        acc <- pacc
        pure (a :. acc))
    (valueParser Nil)
...
firstNameParser = do
    ...
    v <- list lower
    pure (u :. v)
-- firstNameParser = lift2 (:.) upper (list lower) -- (can convert do to lift2 because u and v not used on the right of an <-)
...
surnameParser = do
    ...
    w <- list lower
    pure (u :. v ++ w)
-- surnameParser = lift3 (\u t l -> u :. t ++ l) upper (thisMany 5 lower) (list lower)
...
instance Functor Parser where
  (<$>) ::
    ...
    -> Parser a
    -> Parser b
  f <$> pa = bindParser (\x -> valueParser (f x)) pa
